...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully aggregated plays for ${users.length} users`,
    });
  } catch (error) {
    console.error("Error aggregating plays:", error);
    return NextResponse.json(
      { success: false, error: `Error aggregating plays: ${error}` },
      { status: 500 }
    );
  }
}
